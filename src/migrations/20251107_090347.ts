...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_posts\`("id", "title", "slug", "content", "featured_image_id", "author_id", "status", "published_date", "updated_at", "created_at") SELECT "id", "title", "slug", "content", "featured_image_id", "author_id", "status", "published_date", "updated_at", "created_at" FROM \`posts\`;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_posts\`("id", "title", "slug", "excerpt", "hero_image_id", "body", "status", "published_at", "author_id", "updated_at", "created_at") SELECT "id", "title", "slug", "excerpt", "hero_image_id", "body", "status", "published_at", "author_id", "updated_at", "created_at" FROM \`posts\`;`)
  await db.run(sql`DROP TABLE \`posts\`;`)
  await db.run(sql`ALTER TABLE \`__new_posts\` RENAME TO \`posts\`;`)
  await db.run(sql`PRAGMA foreign_keys=ON;`)
  await db.run(sql`CREATE UNIQUE INDEX \`posts_slug_idx\` ON \`posts\` (\`slug\`);`)
